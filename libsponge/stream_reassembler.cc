...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
    ...
    if ((unused_bytes = _capacity - _output.buffer_size() - _unassembled_size) <= 0) {
        // if the comming substring has index = _ackno, insert first and then try to
        // assemble the remaining unassembled bytes
        if (index == _ackno)
            _ackno += _output.write(data);
...
//! in an already inserted substring, aborts inserting this substring. Remove
...
//! \details try to assemble the unassembled bytes to _output
void StreamReassembler::try_assemble() {
    while (!empty()) {
        substr_t p = top_unassembled();
        std::string tobe_written = "";
        size_t bytes_written = 0;

        // if the index of the unassembled substring is less or equal than expected
        if (p.first <= _ackno) {
